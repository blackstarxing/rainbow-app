...
var messageLog = new Vue({
  	...
  	methods: {
       ...
       // 图形验证码
            ...
            if(type==0){
                if(_this.phone){
                    if(/^1[34578][0-9]{9}$/.test(_this.phone)){
                        $.ajax({
                            url: '/api/checkCode?phone='+_this.phone,
                            type: 'get',
                            success: function(data) {
                               _this.regPic = '/api/checkCode?phone='+_this.phone+'&rand='+new Date();
                               $('.g-checkCodeMask').show();
                            },
                            error: function() {
        ...
        //改变图形验证码
        changePic:function(){
            this.regPic = '/api/checkCode?phone='+this.phone+'&rand='+new Date();
        },
        // 校验验证码
        checkCode:function(){
            var _this = this;
            ...
            if(_this.code){
                if(_this.code.length<4){
                    ...
                    setTimeout(function(){
                     ...
                     $.ajax({
                        url: '/api/verifyCheckCode?phone='+_this.phone+'&checkCode='+_this.code,
                        type: 'get',
                        dataType: 'json',
                        success: function(data) {
                            if(data.code==0){
